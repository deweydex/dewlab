...
Problems on counting, some on paper and some in code, and three from
earlier pages. Before you start each one, ask two questions:
...
hard part of these problems; the arithmetic after it is the easy part.
...
which is how to check a formula on small numbers. Run this cell once
before you start.
...
$0! = 1$ is a definition: we do not work it out, we agree on it. It is
the only value that keeps every formula on the page working, and it makes
sense too: there is exactly one way to arrange nothing.

</details>

**2.** Can you simplify $\frac{10!}{8!}$ and $\frac{n!}{(n-2)!}$ without
working out the factorials?
...
both the top and the bottom, so they cancel. This cancelling is why a
...
How many digits does $100!$ have? Within ten counts.
```
...
observe is thought to have about 81. Even $20!$ is more than two billion
billion: a program that tries every order of 20 things will not finish.
...
---
1,680 both ways: $8 \times 7 \times 6 \times 5$. The eight letters of
COMPUTER are all different, which keeps this one short. The next problem
...
---
180 different words. Swapping the two Es gives the same word, so every
word is counted twice; the two Ts double it again. The formula divides by
$2!$ for each repeated letter: $\frac{720}{2 \times 2} = 180$. In general,
start with the factorial of the number of letters, and divide by the
factorial of each repeat count.
```
...
56 committees, and 336 ways to fill the posts.
...
of its $3! = 6$ orders: $336 \div 6 = 56 = C(8, 3)$. Dividing by $r!$ is
the whole difference between a permutation and a combination.
...
The row is 1, 5, 10, 10, 5, 1, and the total is 32. The row reads the
same both ways, because choosing 2 to keep is choosing 3 to leave:
$C(n, r) = C(n, n-r)$.
...
If the order mattered, it would be $P(47, 6)$, about 7.7 billion: 720
times more, since each ticket's six numbers come in $6! = 720$ orders.
...
**10.** Can you work out each of the four counts in the question above?
...
does the order matter, and can a choice come again?

</details>

## Your world

**11.** A count from the world you chose.
...
---
671 of 1,296. The shorter way counts what is left out: $5^4 = 625$
outcomes have no six, so $1{,}296 - 625 = 671$ have at least one.
Counting the opposite is often easier than counting "at least one".
...
helped to start the study of probability: the next page.
```
...
`book.groupby("chapter")` goes through the chapters one at a time.
```
...
for pair in itertools.combinations(people, 2):
    if pair not in met:
        print(pair)
---
21 pairs, and 14 of them are named in the same chapter. Each of the
seven that never are puts one of the Time Traveller's dinner guests
(Filby, the Medical Man, the Psychologist) beside someone from the year
802,701: Weena, the Eloi or the Morlocks. The count shows the book's two
times without reading a page of it. Only Weena crosses between them:
in chapter 16, the Medical Man examines the flowers she put in the Time
Traveller's pocket.
```
...
The characters, by this count: $72^{10} \approx 3.7 \times 10^{18}$,
against $10{,}000^4 = 10^{16}$, about 370 times fewer. From a list of
50,000 words, four give $6.25 \times 10^{18}$, slightly more than the
characters.

The count is not the whole story. People can remember four words; ten
random characters get written on a note, and then the password is only
as safe as the note.
...
again, because people meet a rule in the easiest way they can. Counting
...
**14.** Six numbers from 47, where a number may be chosen more than once
and the order does not matter. How many choices are there? The count for
$r$ choices from $n$ with repeats is $C(n + r - 1, r)$. Can you check the
formula on small numbers first, with
...
---
35 both ways for 3 from 5, and $C(52, 6) = 20{,}358{,}520$ for the
lottery with repeats: about twice the real one. Why the formula works:
picture $r$ dots and $n - 1$ bars in one row, $n + r - 1$ places in all.
The bars split the row into $n$ groups, one for each number, and the dots
in a group say how many times that number was chosen. Each choice is one
way to pick which $r$ of the places hold dots.
```

## From earlier

**15.** From *Repeating steps with loops*. This loop means to work out
...
starts at 0 and a product starts at 1: the start is the value that
changes nothing.
...
rows. How many different truth tables are there for two inputs: how many
