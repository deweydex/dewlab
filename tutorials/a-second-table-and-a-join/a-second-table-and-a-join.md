---
title: "Joining two tables: foreign keys and JOIN"
year: "2026-2027"
version: 2026.09.23.1
covers:
  join-querying-across-both-tables:
    covers: [DBM-LO5]
    touches: [DBM-LO9]
---

# Joining two tables: foreign keys and JOIN

Real data rarely fits in one table. A dinosaur is one record. Where its
fossils were found is a different kind of record, and one dinosaur can
have several fossil sites. Splitting the two into separate tables avoids
repeating a dinosaur's name, diet and length once per fossil site. A
`JOIN` is how a query brings the two tables back together.

Let's build `dinosaur_tbl` again, then a second table, `sighting_tbl`,
for fossil sites.

```sql exec
id: create-dinosaurs-table
CREATE TABLE dinosaur_tbl (
    dinosaur_id INTEGER PRIMARY KEY,
    name TEXT,
    diet TEXT,
    length_meters REAL,
    period TEXT
);

INSERT INTO dinosaur_tbl (name, diet, length_meters, period) VALUES
    ('Tyrannosaurus Rex', 'Carnivore', 12.3, 'Late Cretaceous'),
    ('Triceratops', 'Herbivore', 9.0, 'Late Cretaceous'),
    ('Velociraptor', 'Carnivore', 2.0, 'Late Cretaceous'),
    ('Brachiosaurus', 'Herbivore', 25.0, 'Late Jurassic'),
    ('Stegosaurus', 'Herbivore', 9.0, 'Late Jurassic'),
    ('Allosaurus', 'Carnivore', 9.7, 'Late Jurassic');
```

```sql exec
id: create-sightings-table
CREATE TABLE sighting_tbl (
    sighting_id INTEGER PRIMARY KEY,
    dinosaur_id INTEGER,
    location TEXT,
    year INTEGER
);

INSERT INTO sighting_tbl (dinosaur_id, location, year) VALUES
    (1, 'Montana, USA', 1902),
    (1, 'Saskatchewan, Canada', 1981),
    (2, 'Wyoming, USA', 1888),
    (3, 'Mongolia', 1971),
    (5, 'Colorado, USA', 1877);
```

`sighting_tbl.dinosaur_id` holds a value that also appears in
`dinosaur_tbl.dinosaur_id`. That shared value is what connects one
table's row to the other's.

A column like `sighting_tbl.dinosaur_id` is called a *foreign key*: it
holds the key of a row in another table. Notice two things about it.
It has the same name as the key it points at, `dinosaur_id`, so you can
see at once which table it points into. It also sits directly under
`sighting_tbl`'s own key, before the other columns. Every table in this
course is written that way, so its links are always the first thing
you read.

## JOIN: querying across both tables

```sql exec
id: join-dinosaurs-and-sightings
SELECT dinosaur_tbl.name, sighting_tbl.location, sighting_tbl.year
FROM sighting_tbl
JOIN dinosaur_tbl ON sighting_tbl.dinosaur_id = dinosaur_tbl.dinosaur_id
ORDER BY sighting_tbl.year;
```

`JOIN dinosaur_tbl ON sighting_tbl.dinosaur_id = dinosaur_tbl.dinosaur_id`
tells the query which column in each table holds the shared value. For
every row in `sighting_tbl`, it finds the `dinosaur_tbl` row whose
`dinosaur_id` matches, and the result carries columns from both.
`table.column` names a column when two tables in the same query share a
name, the way both of these have a `dinosaur_id`.

## Why this happens

Storing a dinosaur's name on every one of its sighting rows would work.
A misspelling in one row would then disagree with the others, and
nothing would flag it. Storing the name once, in `dinosaur_tbl`, and
referring to it by `dinosaur_id` everywhere else means it can only be
spelled one way. It also means a query has to put the two tables back together
itself, on the shared column, rather than finding them already
combined. `JOIN` is what does that.

## Your turn

Bring your table back from the earlier pages. Paste in the `CREATE
TABLE` and `INSERT` statements you have written so far, then run them
again to rebuild it. If this is your first time in this series, write a
new `CREATE TABLE` and some `INSERT` statements of your own instead.

```sql exec
id: join-your-tables
-- Paste the CREATE TABLE and INSERT statements you have written so far
-- for your own table, then run them here to bring it back.
-- First time in this series? Write a new CREATE TABLE and some INSERT
-- statements of your own instead.
```

In the same box, below what is already there, try adding a `CREATE
TABLE` for a second table that relates to it. If your table is
`film_tbl`, a second table, `actor_tbl`, might hold one row per actor,
with a `film_id` column naming which film they belong to. If your table
lists books, it might hold one row per author. Give the linking column
the same name as the first table's key, and put it directly under the
second table's own key. Insert a few rows, then try
writing a `JOIN` that brings a row from each table together. Run it
here, in this one box.

This is the last page of the series. The code in this box is saved
automatically, like every cell on this site, so you can always come back
and run it again. If you want a copy of your own, to keep or to hand in,
Settings has a few ways to save the whole page, code included.

## What you have now

- **Foreign key** names a row in another table from a column in this
  one, the way `sighting_tbl.dinosaur_id` names a row in
  `dinosaur_tbl`. It takes the same name as the key it points at, and
  sits directly under its own table's key.
- **JOIN** combines rows from two tables in one query, matched on a
  column they share.
