...
`:hover` matches an element while a pointer sits over it. `:focus`
matches whichever element currently has keyboard focus. `transition`
animates a change smoothly, defined on the base state rather than the
