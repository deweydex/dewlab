...
An `and` takes two true-or-false values and gives back one. There are only two possibilities for each input, so there are only four situations in total -- and four is few enough to list.
...
Four rows, generated by a loop rather than typed out. That is the point worth taking from this section: **a truth table is what you get when you try every input, not a thing to memorize** — and trying every input is a nested loop you could already write.
...
The middle one is worth a second look. For true-or-false values, "exclusive or" and "not equal to" are the *same operation* -- exactly one being true is the same as the two being different. It is the same idea named twice by people who arrived at it from different directions, not a coincidence or a trick.
...
There were three operators, four rows each, and two rules for moving a `not` through a bracket.
...
**De Morgan's Laws are for readability.** They make tangled conditions into ones a person can read; they do not make code do anything new, and you will use them mostly on code you wrote yourself two weeks earlier.
