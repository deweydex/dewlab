...
Each answer is hidden until you open it. On the small datasets, try to calculate the statistics by hand. Five numbers do not take long. If you do it by hand once, a formula makes more sense. It feels like more than symbols.

Some of these problems are adapted from the statistics and probability worksheet in the Mathematics repository.
...
It is 13, which is the mean of the two middle values, 12 and 14.
...
The median describes the office better. Nobody in the office earns anything near the mean. The mean would be true, but it would mislead people.
...
**6.** For `[2, 4, 4, 4, 5, 5, 7, 9]`, can you calculate the mean by hand, and then the standard deviation that divides by $n$? (This is also called the population standard deviation.)
...
These numbers were chosen so that everything is a whole number. That almost never happens with real data.
...
When we square a deviation, it is always positive. The square also gives large deviations much more weight. That is a choice, and other choices are possible. We could take the absolute value of each deviation instead, which means we drop its minus sign. That gives the mean absolute deviation. It is a perfectly good measure, but it is harder to work with in algebra.
...
In the first class, almost everyone scored close to 65%. The second class has both very high marks and very low marks.

The mean is the same, but the two classes need very different teaching. This is why we should never report an average alone.

</details>

**9.** What is the difference when we divide by $n$ and when we divide by $n - 1$?

<details class="dl-answer"><summary>answer</summary>

When we divide by $n$, we get the population standard deviation. It measures the spread of the numbers you have.

When we divide by $n - 1$, we get the sample standard deviation. Sometimes our numbers are a sample: a few values taken from a much larger group, called the population. The sample standard deviation is an estimate of the spread of that whole population.

The $n - 1$ is a correction. A sample's own mean is never further from the sample's values than the true population mean is. So the deviations are a little too small. A slightly smaller number on the bottom corrects for that.

For $n = 100$, the two answers differ by about half a percent. For $n = 5$, they differ by more than 10%. People are most tempted to ignore the difference exactly when the sample is small.
...
(e) is the trap. We can calculate the average of shirt numbers, but the answer means nothing. A program will do it without any warning.
...
**16.** Here are ten exam marks: `[45, 52, 68, 71, 71, 74, 78, 82, 89, 95]`. Can you calculate a full summary, and describe the class?
...
One value out of eleven moved the mean by 5.5 marks and the median by 1.5. The standard deviation grew by half. Why did it grow so much? The new mark is 55 below the new mean, and that deviation is squared: $55^2 = 3025$.
...
This is the best reason for the next tutorial. **Plot the data.** A picture can show you something that the summary numbers do not.
