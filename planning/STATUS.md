...
- **Student Notes**: A free-text notes field riding on the same per-tutorial saved-progress record and export/import path as cell code, distinct from a tutorial's own author-written pedagogical notes (`planning/SIDEBAR_CONTENT.md`). Plain version built per `planning/STUDENT_NOTES.md` (DECISIONS_LOG.md 7.72); the export-button staleness marker from that design's larger proposal is still open, tracked in `planning/OPEN_QUESTIONS.md`.
