...
4. **Full-stack — done.** `html app`/`css app`/`js app`, a sixth fence
   kind beside the site editor rather than a third site-pane language,
   since a site editor's sandboxed preview exists specifically to stop a
   reader's script reaching anything else on the page, and this needed
   exactly that channel — the page's own shared `db`. `DECISIONS_LOG.md`
   7.180 has the design: the `_query_rows()`/`queryRows()` bridge across
   the Worker boundary, why it is a separate cell kind, and a real,
   pre-existing gap in the authoring editor's fence round-trip (site
   fences and app fences both) found and closed in the same pass. One
   page ported from dewstack's staged reference, in shape rather than
   copied — dewlab's single shared connection drops the per-cell
   database name dewstack's `dlQuery(name, sql, params)` needed and this
   does not. Registered as `courses/full-stack.yaml`, the current way
   (7.172) — no per-series order file, no repository-wide module list,
   both retired along with the folder-per-module layout. No QQI
   mapping; none exists for this module, the same honest gap `covers:`
   leaves open elsewhere rather than inventing one.
...
| `full-stack` | done (reference only) | done — `html app`/`css app`/`js app`, `_query_rows()`/`queryRows()` Worker bridge (`DECISIONS_LOG.md` 7.180) | done — 1 of 1 staged page (`a-page-that-reads-from-a-database`), no QQI mapping (none exists for this module) | not yet — no homepage card until the module has run in front of a class |
