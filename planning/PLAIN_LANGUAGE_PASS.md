...
flattening would cost the most. `database-methods` and `web-authoring` are
both done — see "Done" above. `mit-pdp-maths-prog-integration`,
`computational-methods` and `fundamentals-of-oop` are still untouched.
