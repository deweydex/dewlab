...
see §7 for exactly what did and didn't come along with them.
...
- **Tutorial and practice pages: the pill and the run line are now ported
  ...
  Still not part of this port, and still dewmini-only: the collapse
  triangle, the header→footer layout move, and Duplicate. Collapse and
  Duplicate need the same cell-type question §7's SQL/HTML/CSS/JavaScript
  bullet already raises — collapsing prose-like custom cells makes sense
  on the same reasoning as dewmini's Text cells, but authored cells are
  static content generated by `build.py`, not something a reader adds or
  duplicates on the page. Moving Run/Reset/⋯ out of the header remains a
  larger, separate change to the primary reading surface, not attempted
  here. The same instinct that makes a Text cell go quiet until touched
  in dewmini should carry over directly, though: a tutorial's own
  prose-like cells (Markdown-shaped, non-executable) ought to fade their
  chrome away the same way, rather than looking like every other
  numbered, run-tracked cell around them.
