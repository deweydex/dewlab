...
**Datasets, kept in the repo, with attribution.** More built than it
looks. `data/` already exists, the build already serves it to every page
(`manifest.dataBase`, `build.py`), and `tutorial_tools.load_csv(name)`
already fetches a CSV from it and returns a DataFrame — datasets live
once and are fetched at runtime, never embedded or copied per tutorial,
per its own docstring. Nothing calls it yet (`grep -r load_csv
tutorials/` finds nothing). So the actual gap is real dataset files with
a source anyone can check, and somewhere for attribution to live that
survives the dataset being used by more than one tutorial.

**Pedagogical notes / additional context.** Nothing exists for this
today. The closest relative is the hint/answer fold (`FOLD_CLASSES`,
...
underlying trick — raw HTML inside markdown, checked at build time — is
worth reusing.
...
but per §3 below, that doesn't need its own mechanism if notes already
have one, since a note that contains an image is just markdown content,
and markdown content already renders an image correctly wherever it is
placed. No YouTube or other video embeds in this pass.

## 2. Datasets: no new markdown syntax needed at all

This one does not need an authoring-mechanism decision, because a
dataset is not prose bound to a point in a tutorial's flow — it is a
fact about a file. Two small additions, neither touching the fence/cell
machinery:
...
- **One `.yaml` per dataset** — `data/<name>.yaml` beside `data/<name>.csv`
  ...
  description per file — `dataset_attribution()`. One place attribution
  lives once, however many tutorials end up using that file.
- **A `datasets:` frontmatter list** on a tutorial (`datasets: [life-expectancy]`),
  ...
  declarative, not scraped from cell code, which breaks the moment a
  dataset name is built from a variable instead of a literal.
...
(The blank lines after `<aside ...>` and before `</aside>` matter — that
is what makes most markdown converters still process the inside as
markdown rather than treating it as opaque HTML, the same reason the
existing hint/answer folds are written the same way.) Extracted the same
way `check_folds()` already scans `body_html` for `<details>` tags with a
known class.
...
**Recommendation: (b), the HTML aside.** It is cheaper, and, more
importantly, is not a new pattern — it is the same pattern the fold
feature already proved works, already documented, already has a
build-time check to model the new one on: one fewer kind of thing for the
next person reading `build.py` to learn.

## 4. Where notes (and datasets) surface

The user's own phrasing floated this directly: pedagogical notes "or,
better yet, in the other side bar" — i.e. not necessarily a *third*
panel. Two real options:
...
- **A second section inside the existing reference panel** — "This
  ...
  (both **not** cumulative — a note is tied to this specific tutorial,
  not something every later tutorial keeps carrying, the way a glossary
  term does). Same toggle, same panel, no new open/close/mutual-exclusion
  wiring — `renderReference()` already groups by kind; grouping by
  section is the same idea one level up.

**Settled: extend the existing panel**, for the same reuse-over-invention
reason as §3, and because it directly answers what was asked for ("in
the other side bar"). The one thing this costs is conceptual: the panel
stops being *only* "what has this reader been taught" and starts being
"everything about this tutorial worth having to hand" — worth naming
clearly in the UI (a heading per section) so a reader does not read a
note as if it were a taught, examinable term.
...
- An external change (PR #65, not part of this plan) moved the reference
  panel itself from right-anchored to left-anchored. The series nav panel
  shares that left anchor, stacked below its toggle, and joined the
  Settings/reference mutual-exclusion group as a third member.
...
Is a *left*-anchored panel worth having at all, for something other than
notes/datasets? Settled: yes, for navigation — a different purpose from
either side of §4's choice, and one that doesn't compete with it. §4's
"everything about this tutorial worth having to hand" (reference, notes,
datasets) answers "what do I need right now"; a left panel answering
"where am I, and where can I go" is a genuinely different question, and
conflating the two into one panel would make each harder to scan.
...
long page — moved into the panel instead of, or in addition to, that
inline version) plus the series' contents — the same list `nav_for()`'s
...
into the masthead's own action row alongside Settings' (rather than being
fixed-position buttons stacked in the page's top-left corner), and all
three panels dock flush to their edge, full viewport height, rather than
floating as inset cards. The reference/series-nav mutual exclusion is
unchanged in behaviour — only the mechanism changed, from "a shared
corner" to "a shared edge." Not revisited: whether they should merge into
one panel with two sections — that reasoning doesn't depend on which
shape either panel takes.

## 5. What this does not decide

- Exact wording/heading for the panel once it holds more than a
  cumulative glossary (still called "Reference"? Something broader?) — a
  naming question, cheap to change later either way.
- Whether every tutorial needs a note, or whether most will have none —
  the same "a missing file/field means empty, not an error" shape the
  reference already established: an empty `notes:`/no `<aside>` tags is
  simply nothing added to the panel.
- The `.glossary.yaml`-writing skill's counterpart for notes and
  datasets — out of scope here on purpose. Notes are authorial judgment
  about what deserves a call-out, a much worse fit for a mechanical
  extraction skill than the glossary was.
