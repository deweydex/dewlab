...
**`named-grid-areas`' two drawn maps become one grid the reader resizes**, laid out by the tutorial's own two `grid-template-areas`, with a container query at the same 350px the page's media query uses. `resize: horizontal` was the first attempt and the nicer gesture, and it is mouse-only — on a phone the reading column is narrow enough that a reader would never reach the wide map at all, so the diagram would quietly show half of itself to anybody on a handset. Two radios set the width instead: clickable, touch-friendly, keyboard-reachable, no JavaScript, and the same checkbox hack `keyframes-and-the-checkbox-hack` teaches. **The two cannot be offered together.** The radios have to carry `!important` or they go dead the first time anyone drags and leaves an inline width behind — and that same `!important` is what stops the dragging. Buttons win because they work everywhere; the continuous version is the live preview higher up the same page, which now reads out pixels.
...
*Cost to change: low for the diagrams, each self-contained markup and CSS. The width readout touches `assets/tutorial-runtime.js` and so the committed vendor bundle, which is the one change here that cannot be reverted by editing a tutorial — but it is additive: the slider behaves as before for any page that ignores the number.*
