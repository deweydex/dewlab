...
dewmini-only for now (7.112 closes that gap). One thing not left for
...
**7.112 — Collapse and Duplicate, the rest of dewmini's cell anatomy,
ported to tutorial and practice pages.** 7.111 carried over the pill and
...
**Corrected while writing this: 7.111's own text overstated what was
...
side that had to move to match this one. Fixed in 7.111's own entry
along with `planning/CELL_IDENTITY.md` §7, since both were still on this
same, not-yet-merged branch.
