...
**Seams.** Predict blocks take the places where Python and maths disagree: `3 ^ 2` is 1, `-3 ** 2` is $-9$, `x = x + 1` prints 6, `(x + 3) ** 2` is not `x ** 2 + 9`, and `v - u / a` is not $\frac{v - u}{a}$. Their notes will link to the closer-look pages when #337 writes them. A number predict is compared with the last number the cell prints, so each of these cells ends on the line its question asks about.
