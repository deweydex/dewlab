...
**7.185 — Three settled planning docs deleted outright, not archived; every citation to them stripped from the code, not repointed.** Josh: past planning that no longer describes an open question is not worth the read, git already keeps it, and a citation to a closed document is dead weight in the same way the document itself was.
