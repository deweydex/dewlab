...
export function restoreExecTag(markdown) {
  ...
  // Crepe-based authoring editor, which otherwise keeps only the first
  // word of a fence's info string (ARCHITECTURE.md §3, "The authoring
  // editor"). `python exec`/`sql exec` recover from a lone `id:` header
  // line; a site editor or full-stack cell's `html`/`css`/`js` pane
  // needs two header lines — `id:` plus `site:` or `app:` — since the
