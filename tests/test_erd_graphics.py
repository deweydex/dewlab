...
TIMETABLE = """
CREATE TABLE programme_tbl (programme_id INTEGER PRIMARY KEY, name TEXT);
CREATE TABLE teacher_tbl   (teacher_id INTEGER PRIMARY KEY, name TEXT);
CREATE TABLE room_tbl      (room_id INTEGER PRIMARY KEY, name TEXT, capacity INTEGER);
CREATE TABLE module_tbl    (module_id INTEGER PRIMARY KEY, programme_id INTEGER, name TEXT,
    FOREIGN KEY (programme_id) REFERENCES programme_tbl(programme_id));
CREATE TABLE session_tbl   (session_id INTEGER PRIMARY KEY, module_id INTEGER,
    teacher_id INTEGER, room_id INTEGER, session_date TEXT,
...
class TestReadingASchema:
    def test_a_declared_foreign_key_is_reported(self):
        ...
        modules = {c["name"]: c for c in schema["module_tbl"]["columns"]}
        assert modules["programme_id"]["references"] == ("programme_tbl", "programme_id")
        assert modules["module_id"]["primary"] is True
...
class TestLayout:
    def test_a_table_sits_to_the_right_of_what_it_points_at(self):
        ...
        assert depth["programme_tbl"] == 0
        assert depth["module_tbl"] == 1
        assert depth["session_tbl"] == 2
...
class TestTheDrawing:
    ...
    def test_an_edge_reaching_past_a_column_does_not_cross_it(self):
        """`room_tbl` points at `session_tbl` two columns away. The straight line
    ...
    def test_an_edge_runs_from_the_key_row_to_the_foreign_key_row(self):
        """A line from a table's name band would say the relationship
        belongs to the whole table. It leaves the parent's own key row and
        arrives at the child's foreign-key row: `programme_id` to
        `programme_id`, with `module_id` sitting between the two ends so a
        line from the wrong row would show up here."""
        import re
        ...
        foreign_row = erd._row_centre(child, schema["module_tbl"], 1)
        edges = [
            [tuple(float(v) for v in pair.split(",")) for pair in points.split()]
            for points in re.findall(r'points="([^"]+)"', erd.render(schema))
        ]
        # The edge drawn into module_tbl's left side: its last point sits
        # just short of that box.
        edge = next(e for e in edges if e[-1][0] == pytest.approx(child["x"] - 14.4))
