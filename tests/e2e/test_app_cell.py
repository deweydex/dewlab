...
def test_the_html_and_css_panes_are_live_before_run_is_pressed(page):
    ...
    panes follow. No reader-list items yet, since that only exists once
    the js pane's own query has written them in."""
...
def test_clear_reverts_the_editors_and_clears_the_preview(page):
    seed_readers(page)
    run_app_cell(page)
    page.wait_for_selector(f"{APP_SELECTOR} #reader-list li")
