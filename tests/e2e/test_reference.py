...
class TestVisibility:
    def test_no_glossary_anywhere_in_the_series_still_shows_math_basics(
            self, site, browser, base_url):
        """Math Basics (build.py's load_math_basics()) is the same on
        every page, so the toggle no longer hides just because this page
        and its series have nothing of their own yet — only the
        Reference tab itself says so."""
