...
class TestAppCells:
    """A full-stack cell (`html app`/`css app`/`js app`): a site editor's
    fence grammar and grouping rule, reused for a different purpose — its
    HTML/CSS render straight into the page rather than a sandboxed iframe,
    and its JS pane can read the page's own shared SQL connection. See
    build.py's render_app_cell() and planning/DEWSTACK_MERGE.md §3, §7
    phase 4."""

    def test_a_solo_js_pane_becomes_a_cell(self, repo):
        write(repo, APP_JS)
        b.build()
        page = built(repo)
    ...
    def test_consecutive_panes_of_the_same_app_share_one_cell(self, repo):
    ...
    def test_every_app_cell_gets_a_run_and_a_reset_button(self, repo):
        write(repo, APP_JS)
        b.build()
        page = built(repo)
        assert "dl-btn-app-run" in page
        assert "dl-btn-app-reset" in page
    ...
    def test_the_starter_code_travels_in_the_manifest_not_the_dom(self, repo):
        ...
        b.build()
        page = built(repo)
        assert 'console.log("hi")' not in page  # not embedded as escaped text
        cells = manifest(page)["appCells"]
        assert cells == [{"name": "reader", "panes": {"js": {"id": "reader-js", "code": 'console.log("hi");'}}}]

    def test_a_tutorial_with_no_app_cells_carries_no_manifest_key(self, repo):
        write(repo, CELL)
        b.build()
    ...
    def test_two_different_apps_on_one_page_both_render(self, repo):
        write(repo, APP_JS + "```js app\nid: aside-js\napp: aside\nconsole.log(2);\n```\n")
        b.build()
        page = built(repo)
    ...
    def test_a_cell_between_two_app_cells_does_not_confuse_them(self, repo):
    ...
    def test_an_app_cell_needs_sqlite_even_with_no_sql_cell(self, repo):
        write(repo, APP_JS)
        b.build()
        assert manifest(built(repo))["needsSqlite"] is True
