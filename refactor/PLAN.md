...
### Step 1 — migrate the files (this folder's script)

```bash
python3 refactor/migrate_tutorials.py            # dry run: report only
python3 refactor/migrate_tutorials.py --apply    # writes
```
...
8. Redirect stubs from `courses/redirects.yaml`: a page at each old address
   ...
   `write_redirects()`.
9. The front-page cards come from the course files: `render_index` replaces
   the hand-written card fences in `pages/home.md` with generated ones (the
   `card:` field is the text).
...
`docs/WRITING_TUTORIALS.md` (§"Where a tutorial sits", §"Adding a new
module", the frontmatter table), `ARCHITECTURE.md` §1, `CONTRIBUTING.md`,
`CLAUDE.md` ("Cell ids are a contract" gains "and so is an id"),
`docs/build-explained.md`, `docs/FOR_STUDENTS.md` (addresses),
`planning/CURRICULUM_MAP.md` (regenerated by `dev/curriculum_map.py`),
`DECISIONS_LOG.md` 7.172, and `planning/PLAIN_LANGUAGE_PASS.md` if any
student-facing string changed ("Also part of …" is one).

Done when: `dev/check_doc_links.py` passes and nothing in `docs/` says
`tutorials/<module>/`.
